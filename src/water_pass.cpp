...
WaterReflectionPass::WaterReflectionPass(Application* app, const std::string& name) : RenderPass(name), mApp(app) {
    ...
    mRenderTarget = new Texture{resource.device,
    ...
    mDepthTexture = new Texture{resource.device,
                                static_cast<uint32_t>(1920),
                                static_cast<uint32_t>(1022),
                                TextureDimension::TEX_2D,
                                WGPUTextureUsage_RenderAttachment | WGPUTextureUsage_TextureBinding,
    ...
    wgpuQueueWriteBuffer(resource.queue, mDefaultCameraIndex.getBuffer(), 0, &cidx, sizeof(uint32_t));
    ...
    layout = mDefaultCameraIndexBindgroup.createLayout(resource, "water refletcion bind group layout");
    ...
    mDefaultClipPlaneBG.addBuffer(0,  //
                                  ...
                                  sizeof(glm::vec4));
    mDefaultClipPlaneBG.createLayout(resource, "water clip plane clip plane bindgroup layout");
    ...
    wgpuQueueWriteBuffer(resource.queue, mDefaultClipPlaneBuf.getBuffer(), 0, glm::value_ptr(mDefaultPlane),
                         sizeof(glm::vec4));
...
void WaterReflectionPass::createRenderPass(WGPUTextureFormat textureFormat) {
    ...
    mRenderPipeline->setShader(RESOURCE_DIR "/shaders/water_reflection.wgsl", resource);
...
WaterRefractionPass::WaterRefractionPass(Application* app, const std::string& name) : RenderPass(name), mApp(app) {
    ...
    mDefaultClipPlaneBG.addBuffer(0,  //
                                  BindGroupEntryVisibility::VERTEX_FRAGMENT, BufferBindingType::UNIFORM,
                                  sizeof(glm::vec4));
    mDefaultClipPlaneBG.createLayout(mApp->getRendererResource(),
                                     "water refraction clip plane clip plane bindgroup layout");
    ...
    mDefaultClipPlaneBG.create(mApp->getRendererResource(), mDefaultClipPlaneBGData);
}
...
WaterPass::WaterPass(Application* app, const std::string& name) : RenderPass(name), mApp(app) {
    ...
    mBindGroupLayout =
        mWaterTextureBindGroup.createLayout(mApp->getRendererResource(), "water refletcion bind group layout");

    mWaterTextureBindGroup.create(mApp->getRendererResource(), mWaterTextureBindngData);
}
...
void WaterPass::createRenderPass(WGPUTextureFormat textureFormat) {
    ...
    mRenderPipeline->setShader(RESOURCE_DIR "/shaders/water.wgsl", mApp->getRendererResource())
